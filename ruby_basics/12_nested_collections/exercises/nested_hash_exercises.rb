def find_language_information(languages, language_name)
  # Take languages (a nested hash) and language_name as a symbol, return the value for the language_name key (which will be another hash!)
...
def add_information_about_language(languages, language_name, info_key, info_value)
  # Take languages and add the key/value pair info_key/info_value to the nested hash of language_name, then return the updated languages hash
  languages[language_name][info_key] = info_value
  languages
end

def add_language(languages, language_name, language_info_value)
  # Take languages and add the key/value pair language_name/language_info_value to it, then return languages
...
def find_beautiful_languages(languages)
  # Take languages and return a hash containing only languages which have the key/value pair { is_beautiful?: true } listed in their information
  languages.each do |language, info|
    if info[:is_beautiful?] == false
...
def find_language_facts(languages, language_name, fact_index = 0)
  # Take languages (now with additional facts added to each language with the key :facts and value of an array of strings) and return the fact language_name has at fact_index of its facts array, or at index 0 if this argument is not given
  ...
  #                 is_beautiful?: false }
  # }
  if languages[language_name] == nil
    return nil
  else
